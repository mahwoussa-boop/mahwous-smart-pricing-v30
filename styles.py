...
def comp_strip(all_comps):
    """شريط المنافسين المطابقين — يعرض المنتج المطابق عند كل منافس (score≥60%) فقط، مرتبين من الأرخص.
    البيانات مُصفّاة مسبقاً من _row() — لا تظهر هنا منتجات غير متطابقة أبداً."""
    if not all_comps or not isinstance(all_comps, list) or len(all_comps) == 0:
        return ""
    # ── فلتر دفاعي: لا تعرض منتجات score<60% حتى لو وصلت خطأً ──────────
    _MIN_SCORE = 60.0
    filtered_comps = [
        c for c in all_comps
        if isinstance(c, dict) and float(c.get("score", 0) or 0) >= _MIN_SCORE
    ]
    if not filtered_comps:  # fallback إذا كلهم تحت الحد
        filtered_comps = all_comps
    # الترتيب من الأرخص (يأتي مرتباً لكن نُعيد دفاعياً)
    sorted_comps = sorted(filtered_comps, key=lambda c: float(c.get("price", 0) or 0))
    rows = []
    for i, cm in enumerate(sorted_comps):
        ...
        lbl    = "المتصدر" if is_leader else f"#{i+1}"
        bg     = "rgba(255,152,0,.10)" if is_leader else "rgba(108,99,255,.05)"
        border = "#ff9800" if is_leader else "#333366"
        store_color = "#ffb74d" if is_leader else "#9e9eff"
        price_color = "#ff9800" if is_leader else "#9e9eff"

        # شارة درجة التطابق
        if c_score >= 90:
            score_bg, score_c = "rgba(0,200,83,.15)", "#00C853"
        elif c_score >= 70:
            score_bg, score_c = "rgba(255,214,0,.12)", "#FFD600"
        elif c_score > 0:
            score_bg, score_c = "rgba(255,23,68,.12)", "#FF5252"
        else:
            score_bg, score_c = "transparent", "transparent"

        score_badge = (
            f'<span style="background:{score_bg};color:{score_c};font-size:.62rem;'
            f'font-weight:700;padding:1px 5px;border-radius:8px;border:1px solid {score_c}44">'
            f'{c_score:.0f}%</span>'
        ) if c_score > 0 else ""

        short_pname = (c_pname[:55] + "…") if len(c_pname) > 55 else c_pname
        ...
        if purl.startswith("http"):
            ...
            pname_html = (
                ...
                f'style="color:#bbb;font-size:.72rem;text-decoration:underline;'
                f'text-underline-offset:2px;overflow:hidden;text-overflow:ellipsis;'
                f'white-space:nowrap;max-width:270px;display:inline-block" '
                f'title="{title_esc}">{html.escape(short_pname, quote=False)}</a>'
            )
        else:
            pname_html = (
                f'<span style="color:#bbb;font-size:.72rem;overflow:hidden;'
                f'text-overflow:ellipsis;white-space:nowrap;max-width:270px;'
                f'display:inline-block" title="{title_esc}">'
                f'{html.escape(short_pname, quote=False)}</span>'
            )

        _thumb_url = (cm.get("thumb") or cm.get("image_url") or cm.get("image") or "")
        thumb_html = _lazy_img_tag(_thumb_url, width=22, height=22, alt="") if _thumb_url else ""
        ...
        rows.append(
            ...
            f'<div style="display:flex;align-items:center;gap:6px;flex:1;min-width:0;overflow:hidden">'
            f'{thumb_html}'
            f'<span style="font-size:.62rem;color:#666;white-space:nowrap">{lbl}</span>'
            f'<span style="font-weight:700;color:{store_color};font-size:.78rem;white-space:nowrap">'
            f'{crown}{html.escape(c_store, quote=False)}</span>'
            f'<span style="color:#555;font-size:.65rem">→</span>'
            f'{pname_html}'
            f'</div>'
            f'<div style="display:flex;align-items:center;gap:6px;white-space:nowrap;flex-shrink:0">'
            f'{score_badge}'
            f'<span style="font-weight:900;color:{price_color};font-size:.88rem">'
            f'{c_price:,.0f} ر.س</span>'
            f'</div>'
            f'</div>'
        )

    header = (
        '<div style="font-size:.68rem;color:#666;padding:4px 10px 2px;letter-spacing:.03em">'
        '🏪 منافسون مطابقون — مرتبون من الأرخص:</div>'
    )
    return f'<div class="comp-strip" style="flex-direction:column;gap:2px">{header}{"".join(rows)}</div>'
