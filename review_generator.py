...
class ReviewGenerator:
    ...
    def generate_reviews(self, product_name, price, category='oriental',
        ...
        while len(reviews) < count and attempts < max_attempts:
            ...
            if not text:
                continue
            # ⚠️ الطول يُقاس على النص **الخام** عمداً — لا تُطبّعه هنا.
            # مراجعة خارجية اقترحت قياسه على المُطبَّع بحجّة أن الإيموجي يزيد
            # عدد الكلمات فيهرّب نصاً قصيراً من سقف التكرار. جُرّب فعلياً
            # وقيس الأثر، فتبيّن أن الاقتراح يعكس الغرض:
            #   • χ² لمطابقة توزيع الطول: 2.96 (مطابق) ← 155.05 (انحراف حاد)
            #   • أقصى تكرار لنص مُطبَّع: 26 ← 2
            # بينما بيانات المنافس الحقيقية (452 نصاً) فيها أقصى تكرار **28**
            # لنص واحد ونسبة تكرار 21%. أي أن «الثغرة» كانت في الحقيقة صمّام
            # الأمان الذي يسمح للمولّد بإعادة إنتاج تكرار القصير الواقعي؛
            # وإغلاقها جعل المخرجات أنقى من الواقع وكسر المطابقة — وهي سلوك
            # محميّ صراحةً في CLAUDE.md. الهدف هنا مطابقة المنافس لا صفر تكرار.
            if len(text.split()) <= 4:
