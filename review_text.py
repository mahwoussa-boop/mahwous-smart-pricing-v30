...
# زوايا بديلة صريحة تُعرَض بعد عدّة محاولات فاشلة متتالية — «غيّر الزاوية»
# وحدها تعليمات مبهمة لا تكسر نمطاً متكرراً فعلياً؛ قائمة خيارات ملموسة
# تفعل. لا تُفرض كموضوع إجباري، فقط كأمثلة توسّع خيال النموذج بعيداً عمّا
# جرّبه بالفعل عدّة مرات.
_ESCALATION_ANGLES = [
    'تفصيلة حسّية محدّدة (لون العلبة، ملمس القماش، حجم القارورة)',
    'موقف زمني واحد (أول استخدام، بعد الشحن، في مناسبة معيّنة)',
    'مقارنة عابرة بعطر أو منتج آخر جرّبته من قبل',
    'ردة فعل شخص آخر لاحظها',
    'شعور غير متوقّع (خيبة أمل بسيطة، تفاجؤ، ارتياح)',
]


def _retry_hint(attempt, rejected):
    """تلميح إعادة المحاولة — يتصاعد مع تكرار الفشل بدل تكرار نفس التحذير.

    محاولات 1-2: يذكر النصوص المرفوضة حرفياً (يمنع إعادة إنتاج نفس البادئة).
    من المحاولة 3: يضيف قائمة زوايا بديلة ملموسة — «غيّر الزاوية» وحدها لا
    تكفي عملياً حين يكون النموذج عالقاً في نفس الفكرة عبر عدّة محاولات؛
    خيارات محدّدة تكسر النمط فعلاً بدل تكرار تحذير عام لا يتغيّر.
    """
    ...
    if attempt >= 3:
        angles = random.sample(_ESCALATION_ANGLES, k=min(3, len(_ESCALATION_ANGLES)))
        base += ('\nما زلت تكرر نفس الفكرة رغم التحذير — اختر واحدة من هذه '
                 'الزوايا فقط ولا تخلط بينها: ' + ' / '.join(angles))
